	...
	, m_fps(25)
{
	...
	m_tbid.fShowIcon = false; // (theApp.m_AppName.Find(_T("zplayer"), 0) < 0 || !!theApp.GetProfileInt(ResStr(IDS_R_GENERAL), ResStr(IDS_RG_ENABLEZPICON), 0));
...
{
	//MessageBox(0, L"InitSubPicQueue", L"", 0);
	CAutoLock cAutoLock(&m_csQueueLock);
	...
	//CString str;
	//str.Format(_T("m_spd.type: %d m_w %d m_h %d window_w %d window_h %d video_rect_w %d video_rect_h %d \n"), m_spd.type, m_w, m_h, window.cx, window.cy, video_rect.Width(), video_rect.Height());
	//MessageBox(0, str, L"", 0);
...
STDMETHODIMP CDirectVobSubFilter::put_TextSettings(LOGFONTW* lf, int lflen, COLORREF color, bool fShadow, bool fOutline, bool fAdvancedRenderer)
{
...
HRESULT CDirectVobSubFilter2::CheckInputType(const CMediaType* mtIn)
{
	...
	//if (!m_skipAutoloadCheck) if(!ShouldWeAutoload(m_pGraph)) return VFW_E_TYPE_NOT_ACCEPTED;
...
STDMETHODIMP CDirectVobSubFilter::SkipAutoloadCheck(int val)
{
	m_skipAutoloadCheck = (bool)val;
	return S_OK;
}

///////////////////////////////////////////////////////////////////////////////
STDMETHODIMP CDirectVobSubFilter::OpenSubtitle(WCHAR* fn)
{
	...
	int prevStreamCount = m_pSubStreams.GetCount();
	...
	{
		POSITION p;
		if(p = m_frd.files.Find(ret[i].full_file_name)) {
			pSubStream = m_pSubStreams.GetNext(p);
			break;
		}
...
void CDirectVobSubFilter::SetSubtitle(ISubStream* pSubStream, bool fApplyDefStyle)
{
    ...
    if(m_simple_provider) {
		//MessageBox(0, L"m_simple_provider", L"", 0);
