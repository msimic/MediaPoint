...
HRESULT CWndShadow::CreateInstance(IUnknown *pUnkOuter, REFIID iid, void **ppv)
{
    ...
    CWndShadow *pObject = new CWndShadow();
...
HRESULT CWndShadow::QueryInterface(REFIID riid, void ** ppv)
{
    CheckPointer(ppv, E_POINTER);

    if (riid == __uuidof(IUnknown))
    {
        *ppv = static_cast<IUnknown*>( static_cast<IWindowShadow*>(this) );
    }
    else if (riid == __uuidof(IWindowShadow))
    {
        *ppv = static_cast<IWindowShadow*>(this);
    }
	else
    {
        *ppv = NULL;
...
ULONG CWndShadow::AddRef()
{
    return RefCountedObject::AddRef();
}

ULONG CWndShadow::Release()
{
    return RefCountedObject::Release();
}

HRESULT CWndShadow::CreateForWindow(HWND window)
{
	Create(window);
	SetColor(RGB(0,0,0));
	SetPosition(0,0);
	this->SetSize(10);
	this->SetDarkness(255);
	this->SetSharpness(10);
	return S_OK;
}

HRESULT CWndShadow::SetShadowSize(int size)
{
	SetSize(size);
	return S_OK;
}

HRESULT CWndShadow::UpdateWindow(HWND window)
{
	Update(window);
	return S_OK;
}

HRESULT CWndShadow::Init(HINSTANCE hInstance)
{
...
// 			_RPTF2(_CRT_WARN, "%d %d\n", ptAnchorsOri[i][0], ptAnchorsOri[i][1]);
	...
	int skipCnt = 0;
	int lastLine = max(szShadow.cy - nKernelSize, min(szParent.cy + m_nSize - m_nyOffset, szParent.cy + 2 * m_nSize));
		...
		i++)
	{
		...
		else
		{
			for(int j = ptAnchors[i][0];
				...
				j++)
				*(pLine + j) = clCenter;
			for(int j = max(ptAnchorsOri[i - m_nSize + m_nyOffset][0] + m_nSize - m_nxOffset, 0)+3;
				j < min(ptAnchorsOri[i - m_nSize + m_nyOffset][1] + m_nSize - m_nxOffset, szShadow.cx)-3;
				j++)
				*(pLine + j) = 0;
