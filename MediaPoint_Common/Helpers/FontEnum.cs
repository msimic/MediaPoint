...
namespace MediaPoint.Common.Helpers
{
    ...
    public class FontEnum
    {
        ...
        public int callback1(ref ENUMLOGFONTEX lpelfe, ref NEWTEXTMETRICEX lpntme, int FontType, int lParam)
        {
            ...
            {
                FontType fontType;
                if (FontType == 4 && lpelfe.elfFullName.StartsWith("@") == false && lpelfe.elfScript != "Symbol")
                {
                    fontType = (FontType)FontType;
                    FontFamilies.Add(new FontFamily(lpelfe.elfFullName, fontType) { Script = lpelfe.elfScript, Style = lpelfe.elfStyle, Weight = lpelfe.elfLogFont.lfWeight.ToString(), Charset = lpelfe.elfLogFont.lfCharSet.ToString() });
                }
                else
                {
                    fontType = Helpers.FontType.Unknown;
                }
            }
            catch (Exception e)
            {
        ...
        public static FontCharSet GetDesktopFontCharset()
        {
            FontEnum.FONTSIGNATURE sig = new FontEnum.FONTSIGNATURE();
            IntPtr dc = GetDC(IntPtr.Zero);
            FontCharSet ret = (FontCharSet)GetTextCharsetInfo(dc, ref sig, 0);
