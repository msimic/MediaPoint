...
namespace MediaPoint.Subtitles.Logic
{
    ...
    unsafe public class NikseBitmap
    {
        ...
        {
            if (count <= MediaPoint.Subtitles.Logic.VobSub.Helper.B00000011) // 1-3 repetitions
            {
                WriteOneNibble(buffer, count, lastColor, ref index, ref indexHalfNibble);
            }
            else if (count <= MediaPoint.Subtitles.Logic.VobSub.Helper.B00001111) // 4-15 repetitions
            {
                WriteTwoNibbles(buffer, count, lastColor, ref index, indexHalfNibble);
            }
            else if (count <= MediaPoint.Subtitles.Logic.VobSub.Helper.B00111111) // 4-15 repetitions
            {
        ...
        private void WriteFourNibbles(byte[] buffer, int count, int color, ref int index, bool indexHalfNibble)
        {
            ...
            if (indexHalfNibble)
            {
                ...
                index++;
                byte secondNibble = (byte)((n & MediaPoint.Subtitles.Logic.VobSub.Helper.B00001111) << 4);
            ...
            else
            {
                ...
                buffer[index] = firstNibble;
                index++;
                byte secondNibble = (byte)(n & MediaPoint.Subtitles.Logic.VobSub.Helper.B11111111);
                buffer[index] = (byte)secondNibble;
            }
        }
