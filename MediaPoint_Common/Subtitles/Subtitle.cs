...
using MediaPoint.Subtitles.Logic.Enums;
using System.Linq;
...
namespace MediaPoint.Subtitles.Logic
{
    public class Subtitle
    {
		...
		public IEnumerable<Paragraph> GetParagraphsAt(long frame) {
			return _paragraphs.Where(p => p.StartFrame < frame && p.EndFrame > frame);
		}

		public IEnumerable<Paragraph> GetParagraphsAt(TimeSpan time)
		{
			return _paragraphs.Where(p => p.StartTime.TimeSpan < time && p.EndTime.TimeSpan > time);
		}
